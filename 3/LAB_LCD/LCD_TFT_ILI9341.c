...
#define ROW  240	//Y	  //显示的行数
#define COL  320    //X    //显示的列数
...
void LCD_TFT_ShowChar(uint16_t line,uint16_t column,uint8_t AscNum,uint8_t Font,uint16_t pointColor,uint16_t backColor) //显示一个字符( 指定坐标,字体大小,颜色)-"彩屏LCD_TFT"
{
	uint8_t  AscChar = 0;
    uint8_t  i = 0;
    uint8_t  j = 0;
	...
	for(i = 0; i < VerticalNum; i++)
	{
		...
		{
			if(AscChar & 0x01) LCD_ILI9341_TFT_pixel(column, line, pointColor);  // 画字符处的点
			else  LCD_ILI9341_TFT_pixel(column, line, backColor);  //画空白处的点
...
void LCD_TFT_ShowString(uint16_t column,uint16_t line,uint8_t *ArrayPoint,uint8_t Font,uint16_t pointColor,uint16_t backColor) //显示字符串( 指定坐标,字体大小,颜色)-"彩屏LCD_TFT"
{
	uint16_t x = 0;
	uint16_t y = 0;
...
/****************************************************************************
*函数名-Function:	void LCD_Show_ChFont2424(void)
*描述- Description:		显示1个24*24的汉字-"彩屏LCD_ILI9325"
*输入参数-Input:	(x,y):汉字显示的位置    index:tfont24数组里面的第几个汉字
						pointColor: 汉字画笔颜色;     backColor: 背景颜色;
*输出参数-output:	None
*注意事项-Note：
****************************************************************************/
void LCD_Show_ChFont2424(uint16_t x,uint16_t y,uint8_t index,uint16_t pointColor,uint16_t backColor) //显示1个24*24的汉字-"彩屏LCD_ILI9325"
{
	uint8_t byteNum = 0;
	uint8_t i = 0;
	uint8_t temp;
	...
	for(byteNum = 0; byteNum < 72; byteNum++)//每个24*24的汉字点阵 有72个字节
	{
		...
		{
			if(temp & 0x80) LCD_ILI9341_TFT_pixel(y,x,pointColor); //画汉字的点
			else LCD_ILI9341_TFT_pixel(y,x,backColor);   //画空白点（使用背景色）
			temp <<= 1;
			x ++;
...
/****************************************************************************
*函数名-Function:	void LCD_Show_StandbyPage(void)
*描述- Description:		显示"待机页面"-"彩屏LCD_ILI9325"
*输入参数-Input:	None
*输出参数-output:	None
*注意事项-Note：
****************************************************************************/
void LCD_Show_StandbyPage(void) //显示"待机页面"-"彩屏LCD_ILI9325"
{
	uint8_t  ChNum = 0;
	uint16_t yStart = 0;


	LCD_ILI9341_TFT_cls(Red);  //全屏清屏-"彩屏LCD_ILI9325"

	LCD_ILI9341_TFT_cls(Green);  //全屏清屏-"彩屏LCD_ILI9325"

	LCD_ILI9341_TFT_cls(Blue);  //全屏清屏-"彩屏LCD_ILI9325"
...
void LCD_Show_Tempreture(char tempreture) //显示"待机页面"-"彩屏LCD_ILI9325"
{
	//LCD_TFT_ShowString(40,168,tempreture,FONT1608,Red,GreenYellow);
	LCD_TFT_ShowString(40,168,tempreture,FONT1608,Red,GreenYellow);
}
